...
func New(generator dockertesting.DockerItemGenerator, databaseName string, opts ...Option) *MySQL {
...
func defaultMySQL(generator dockertesting.DockerItemGenerator, databaseName string) *MySQL {
	return &MySQL{
...
func (s *MySQL) RunInDocker(m *testing.M) int {
	...
	mysqlDSN = fmt.Sprintf("root:@tcp(%s:%s)/%s?%s",
		hostIP, hostPort.HostPort, s.DatabaseName, s.dsnParams)
